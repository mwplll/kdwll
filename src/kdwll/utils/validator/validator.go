...
const (
	...
	// 匹配身份证
	IDStr = `(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)`

	// 匹配qq
	qqStr = `[1-9][0-9]{4,}`

	// 匹配车牌号
	plateNumberStr = `[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤` +
		`青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}`
)
...
var (
	...
	//name     		= regexpCompile(nameStr)
	ID     	 		= regexpCompile(IDStr)
	qq       		= regexpCompile(qqStr)
	plateNumber		= regexpCompile(plateNumberStr)
)
...
func IsName(name string) bool {
	return regexp.MustCompile("^\\p{Han}+(\u00B7\\p{Han}+)*?$").MatchString(name)
}

// 验证一个值是否匹配一个身份证号。
func IsID(val interface{}) bool {
	return isMatch(ID, val)
}

// 验证一个值是否匹配一个qq。
func IsQq(val interface{}) bool {
	return isMatch(qq, val)
}

// 验证一个值是否匹配一个车牌号。
func IsPlateNumber(val interface{}) bool {
	return isMatch(plateNumber, val)
}
